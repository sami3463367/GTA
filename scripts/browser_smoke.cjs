...
(async()=>{
 ...
 const before=await page.screenshot({path:'artifacts/browser-menu.png'});
 await page.mouse.click(320,353);
 await page.waitForTimeout(1200);
 await page.keyboard.down('d');await page.waitForTimeout(550);await page.keyboard.up('d');
 const after=await page.screenshot({path:'artifacts/browser-playable.png'});
 if(before.equals(after))throw new Error('Native canvas did not respond to Start/movement input');
